...
export const ProductRequests = () => {
  const [suggestion, setSuggestions] = useState(0)
  ...
  if (sort === 'most comments') {
    sorted.sort((a, b) => {
      let totalA = 0
      if (a.comments) {
        totalA = a.comments.length
        a.comments.forEach((item) => {
          if (item.replies) {
            totalA = totalA + item.replies.length
          }
        })
      }
      let totalB = 0
      if (b.comments) {
        totalB = b.comments.length
        b.comments.forEach((item) => {
          if (item.replies) {
            totalB = totalB + item.replies.length
          }
        })
      }
    ...
    <section className='productsReq'>
      <header>
        <span>
          ...
          <h4> {suggestion} suggestions</h4>
        </span>
        <button className='sortByUp'>
          sort by :{'  '}
          <span
            ref={select}
            onClick={() => {
      ...
      {sorted.map((item) => {
        const { id, title, category, upvotes, description, comments } = item
        let totcomm = 0
        if (comments) {
